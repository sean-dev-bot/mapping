---
semantic: 
schema: dbo
type: table
rows: ~53
primary_key: [DcmlFromTID, DcmlToTID, XferNoteTID, XferType]
---

## Description

<!-- Add description here -->

**Schema**: [[dbo]]

**Primary Key**: Composite key on DcmlFromTID, DcmlToTID, XferNoteTID, XferType

## Columns

| Column | Type | Nullable | PK | Identity | Description |
|--------|------|----------|----|-----------|--------------|
| DcmlFromTID | int |  | 🔑 |  | placeHolder |
| DcmlToTID | int |  | 🔑 |  | placeHolder |
| XferNoteTID | int |  | 🔑 |  | placeHolder |
| XferType | char(1) |  | 🔑 |  | placeHolder |
| XferPct | decimal(11,8) | ✓ |  |  | placeHolder |
| XferIntTypeCode | char(5) | ✓ |  |  | placeHolder |
| LastUpdateID | varchar(20) | ✓ |  |  | placeHolder |
| LastUpdated | smalldatetime | ✓ |  |  | placeHolder |

## Indexes

| Index Name | Type | Unique | Columns |
|------------|------|--------|----------|
| PKjbMasXferXref | CLUSTERED | ✓ | DcmlFromTID, DcmlToTID, XferNoteTID, XferType |
| XIF1250jbMasXferXref | NONCLUSTERED |  | XferNoteTID |
| XIF2896jbMasXferXref | NONCLUSTERED |  | XferIntTypeCode |
| XIF3685jbMasXferXref | NONCLUSTERED |  | DcmlToTID |

## Business Context

- **Purpose**: 
- **Data Source**: 
- **Update Frequency**: 
- **Owner**: 

## Notes

<!-- Add your notes here -->


#database #table #dbo
## Related Reports

| Report Name | Columns |
|------------|----------|
